...
export default function EnhancedAdminActionModal({ 
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<AdminActionTemplate | null>(null);
  ...
  const generateRandomUsername = () => {
    const adjectives = ['Cool', 'Smart', 'Fast', 'Bright', 'Swift', 'Bold', 'Wise', 'Quick', 'Sharp', 'Strong'];
    const nouns = ['User', 'Writer', 'Reader', 'Blogger', 'Creator', 'Thinker', 'Explorer', 'Dreamer', 'Builder', 'Maker'];
  ...
  const fetchTemplates = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/action-templates?actionType=${actionType}&isActive=true`);
  ...
  const handleUnban = async () => {
    ...
    try {
      const endpoint = actionType === 'account_ban' ? '/api/admin/ban-user' : '/api/admin/ban-comment';
      ...
      if (response.ok) {
        toast.success(data.message || `${actionType === 'account_ban' ? 'User unbanned' : 'Comment privileges restored'} successfully`);
        onSuccess?.();
        onClose();
      } else {
  ...
  const getActionIcon = (type: string) => {
    switch (type) {
      case 'account_ban': return <MdPerson className="text-red-500" size={24} />;
      case 'comment_ban': return <MdComment className="text-orange-500" size={24} />;
      case 'username_change': return <MdEdit className="text-blue-500" size={24} />;
      case 'delete_account': return <MdDelete className="text-red-600" size={24} />;
      default: return <MdPerson className="text-gray-500" size={24} />;
    }
  };
  ...
  const getActionLabel = (type: string) => {
    switch (type) {
      case 'account_ban': return 'ban user';
      case 'comment_ban': return 'ban comments';
      case 'username_change': return 'change username';
      case 'delete_account': return 'delete account';
  ...
  const getSuccessMessage = (type: string) => {
    switch (type) {
      case 'account_ban': return 'User banned successfully';
      case 'comment_ban': return 'Comment privileges suspended successfully';
      case 'username_change': return 'Username changed successfully';
      case 'delete_account': return 'Account deleted successfully';
  ...
  const isCurrentlyBanned = actionType === 'account_ban' ? user.isBanned : user.commentBanned;
  const showUnbanSection = (actionType === 'account_ban' || actionType === 'comment_ban') && isCurrentlyBanned;
            ...
            <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="font-medium text-red-800">
                  User is currently {actionType === 'account_ban' ? 'banned' : 'comment-banned'}
                </span>
              </div>
              ...
              <div className="mb-4">
                ...
                >
                  {loading ? 'Processing...' : `Quick Remove ${actionType === 'account_ban' ? 'Ban' : 'Comment Ban'}`}
                </button>
              </div>
              ...
              <div className="space-y-3">
                ...
                >
                  {loading ? 'Processing...' : `Remove ${actionType === 'account_ban' ? 'Ban' : 'Comment Ban'} with Custom Details`}
                </button>
              </div>
            </div>
          )}
            ...
            <>
              ...
              <div className="mb-6">
                ...
                {loadingTemplates ? (
                  ...
                  <div className="text-center py-4 text-gray-500">
                    No templates available for {getActionTitle(actionType).toLowerCase()}.
                  </div>
                ) : (
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <div className="space-y-4">
                  ...
                  >
                    {loading ? 'Processing...' : `Apply Custom ${getActionTitle(actionType)}`}
                  </button>
                </div>
              </div>
            </>
