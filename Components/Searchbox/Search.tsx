...
export default function Search() {
  ...
  return (
    <Div
      className="my-5 flex  items-center justify-center gap-3 px-3"
      ...
      <div className="relative grow md:grow-0" ref={searchRef}>
        <div className="bg-gray-300/50 dark:bg-slate-700/50 rounded-lg w-full md:w-[30rem] flex shadow dark:shadow-slate-700 backdrop-blur-md">
          <form className="search-container inline-block relative w-4/5">
            ...
            <button
              type="submit"
              className="absolute right-5 top-0 bottom-0 opacity-50"
            >
              <FaSearch size={20} />
            </button>
          </form>
                ...
                <div className="flex overflow-y-auto">
                  <div className="flex gap-2 flex-wrap w-full">
                    {Array.from({ length: 3 }).map((_, i) => (
                      <div
                        key={i}
                        className="flex w-full items-center p-2 border border-slate-300 dark:border-slate-500 rounded-md animate-pulse duration-300"
                      >
