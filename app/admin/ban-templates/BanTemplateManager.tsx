...
import { MdSecurity, MdComment, MdPerson } from 'react-icons/md';
import BanTemplateForm from './BanTemplateForm';
...
export default function BanTemplateManager() {
  ...
  const handleDelete = async (templateId: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/ban-templates/${templateId}`, {
        method: 'DELETE',
      });
  ...
  const handleEdit = (template: BanTemplate) => {
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-2">
                  {template.banType === 'account' ? (
                    <MdPerson className="text-red-500" size={20} />
                  ) : (
                    <MdComment className="text-orange-500" size={20} />
                  )}
