...
export async function GET(
  ...
  try {
    ...
    // Get blog analytics
    const analytics = await BlogAnalytics.findOne({ blogId }).populate(
      'blogId',
      'title creator'
    );
...
export async function POST(
  ...
  try {
    ...
    let todayStats = analytics.dailyStats.find(
      (stat: { date: { getTime: () => number } }) =>
    ...
    switch (action) {
      case 'view':
        ...
        if (
          blog &&
          userIdentifier &&
          !blog.uniqueViews?.includes(userIdentifier)
        ) {
