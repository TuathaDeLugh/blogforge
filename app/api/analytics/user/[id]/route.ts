...
export async function GET(
  ...
  try {
    ...
    if (!analytics) {
      ...
      analytics = await UserAnalytics.findOne({ userId }).populate(
        'userId',
        'username name avatar'
      );
    }
    ...
    const userBlogs = await Blog.find({
      creator: userId,
      status: 'published',
    }).select('_id title views usersave share comments createdAt');
    ...
    const response = {
      ...
      blogs: userBlogs.map((blog) => {
        const blogAnalytic = blogAnalytics.find(
          (ba) => ba.blogId._id.toString() === blog._id.toString()
        );
        return {
          ...blog.toObject(),
