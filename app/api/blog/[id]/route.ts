...
export async function PUT(request: Request, { params }: any) {
  try {
    ...
    const originalBlog = await Blog.findById(id).populate(
      'creator',
      'email name username'
    );
    if (!originalBlog) {
    ...
    const isAdminEdit =
      ...
      originalBlog.creator._id.toString() !== session.user.dbid;
    ...
    if (!isAdminEdit) {
      ...
      if (banStatus.isBanned) {
        return NextResponse.json(
          createBanResponse(
            'Your account has been banned. You cannot edit blogs.'
          ),
          { status: 403 }
        );
      }
    ...
    if (isAdminEdit) {
      ...
      try {
        ...
        console.error(
          'Failed to send blog modification notification:',
          emailError
        );
      }
    }
