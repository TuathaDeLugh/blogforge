    ...
    try {
        ...
        if (banStatus.isBanned) {
            return NextResponse.json(
                createBanResponse("Your account has been banned. You cannot create blogs."),
                { status: 403 }
            );
        }
...
export async function PATCH(request: Request) {
    try {
        const { blog, user, comment, action, commid, adminReason } = await request.json();
        ...
        switch (action) {
            ...
            case 'remove': {
                ...
                const commentToRemove = foundBlog.comments.find((c: any) => c._id === commid);
                
                if (commentToRemove) {
                    // Get session to check if admin is removing the comment
                    const session = await getServerSession(authOptions);
                    const isAdminRemoval = session?.user?.isAdmin && 
                    ...
                    if (isAdminRemoval && commentToRemove.user) {
                        ...
                        if (commentAuthor) {
                            ...
                            try {
                                ...
                                console.error('Failed to send comment removal notification:', emailError);
                            }

                            // Also notify blog creator if different from comment author
                            if (foundBlog.creator._id.toString() !== commentToRemove.user.toString()) {
                                ...
                                try {
                                    ...
                                    console.error('Failed to send blog creator notification:', emailError);
                                }
                            }
                        }
                    }
