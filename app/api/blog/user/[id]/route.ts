...
export async function GET(
  ...
  try {
    ...
    if (filter === 'all' || filter == undefined || filter == '') {
      blogs = await Blog.find({ creator: id })
        .sort({ title: sort })
        .populate('creator', '_id username avatar')
        .select('_id title share usersave images status')
        .skip(skip)
        .limit(pageSize);
      totalDocuments = await Blog.countDocuments({ creator: id });
    } else {
      blogs = await Blog.find({ creator: id, status: filter })
        .sort({ title: sort })
        .populate('creator', '_id username avatar')
        .select('_id title share usersave images status')
        .skip(skip)
        .limit(pageSize);
      totalDocuments = await Blog.countDocuments({
        creator: id,
        status: filter,
      });
    }
