...
export async function GET(request : NextRequest, response : NextResponse) {
    ...
    try {
        ...
        const allBlogs = await Blog.find({ status:'published'}).select('title images info createdAt usersave share').exec();
        const blogs = await Blog.find({ status:'published' }).select('title images info createdAt usersave share').sort({ title: 1 }).skip(skip).limit(pageSize);
