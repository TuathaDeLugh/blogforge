...
export async function GET(request: Request, { params }: any) {
  ...
  try {
    ...
    const allBlogs = await Blog.find({ creator: user._id, status: 'published' })
      .select('title images info createdAt usersave share')
      .exec();
    const blogs = await Blog.find({ creator: user._id, status: 'published' })
      .select('title images info createdAt usersave share')
      .sort({ title: 1 })
      .skip(skip)
      .limit(pageSize);
