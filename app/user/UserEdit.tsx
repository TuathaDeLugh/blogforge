    ...
    onSubmit: async (values, action) => {
      ...
      try {
        const response = await fetch(`/api/user/${user.dbid}`, {
          ...
          body: JSON.stringify({ ...values, email: user.email, type: 'info' }),
        });
  ...
  return (
    <>
      <Animation>
        ...
        {formik.values.newMail &&
          formik.values.newMail !== user.email &&
          formdisable == true && (
            <Div
            ...
            >
              <p className="text-orange-500">
                New Email needs to be verified. We have sent a verification
                email. If you have not received it or it has expired, please
                request a new one.
              </p>
              <button
          ...
          {user.twoFactorEnabled && (
            <div className="mt-3 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md">
              <p className="text-sm text-green-800 dark:text-green-400">
                ✅ Two-Factor Authentication is active. You'll receive
                verification codes via email when logging in.
              </p>
            </div>
          )}
        </div>
