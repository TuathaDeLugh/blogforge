  ...
  return (
    <div className="py-4 lg:py-6 max-w-[1500px] mx-auto px-4 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg mb-8 text-center relative">
        <ShareButton
          link={`${process.env.API_URL}writer/${encodeURIComponent(writer.user.username)}`}
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg shadow ">
            ...
            <p className="text-gray-800 dark:text-gray-400">
              {new Date(writer.user.createdAt).toLocaleDateString('en-IN', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
          ...
          <div className="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg shadow ">
            ...
            <p className="text-gray-800 dark:text-gray-400">
              Saves: {writer.stats.totalSaves}, Shares:{' '}
              {writer.stats.totalShares}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow ">
          ...
          <Link
            href={`/blogs/${writer.blogs.mostShared._id}`}
            className="block"
          >
            <Image
              height={100}
              width={100}
              src={writer.blogs.mostShared.images[0].link}
              alt={writer.blogs.mostShared.title}
              className="w-full h-48 object-cover rounded-lg mb-4"
            />
            <h4 className="text-lg font-bold text-orange-500 dark:text-orange-400">
              {writer.blogs.mostShared.title}
            </h4>
            <p className="text-gray-600 dark:text-gray-400">
              {writer.blogs.mostShared.info}
            </p>
          </Link>
        </div>
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow ">
          ...
          <Link href={`/blogs/${writer.blogs.mostSaved._id}`} className="block">
            <Image
              height={100}
              width={100}
              src={writer.blogs.mostSaved.images[0].link}
              alt={writer.blogs.mostSaved.title}
              className="w-full h-48 object-cover rounded-lg mb-4"
            />
            <h4 className="text-lg font-bold text-orange-500 dark:text-orange-400">
              {writer.blogs.mostSaved.title}
            </h4>
            <p className="text-gray-600 dark:text-gray-400">
              {writer.blogs.mostSaved.info}
            </p>
          </Link>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full bg-white dark:bg-gray-800 rounded-lg">
            <thead>
              <tr>
                <th className="py-2 px-4 border-b dark:border-gray-700 dark:text-gray-300 text-left">
                  Title
                </th>
                <th className="py-2 px-4 border-b dark:border-gray-700 dark:text-gray-300 text-left">
                  Image
                </th>
                <th className="py-2 px-4 border-b dark:border-gray-700 dark:text-gray-300 text-center">
                  Saves-Share
                </th>
                <th className="py-2 px-4 border-b dark:border-gray-700 dark:text-gray-300 text-center hidden md:table-cell">
                  Date
                </th>
              </tr>
            </thead>
            <tbody>
              {writer.blogs.all.data.map((blog: any) => (
                <tr
                  key={blog._id}
                  className={
                    'even:bg-gray-100 even:dark:bg-gray-700 odd:bg-white odd:dark:bg-gray-800'
                  }
                >
