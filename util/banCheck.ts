...
export async function checkUserBanStatus(userId: string) {
  try {
    await connectdb();
    const user = await User.findById(userId).select('isBanned banExpiry commentBanned commentBanExpiry isActive');
    ...
    if (user.commentBanned && user.commentBanExpiry && new Date() > user.commentBanExpiry) {
      await User.findByIdAndUpdate(userId, {
        commentBanned: false,
